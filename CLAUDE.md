...
- **Drilling Animation** — Both vertical and horizontal drilling is animated with variable duration based on depth and ore type: 0.8s for dirt at ground level, up to 30s at max depth. Ore hardness multipliers (1.2x-3.0x) increase duration. Player progressively moves to tile center via lerp. Tile only removed on animation completion. All inputs blocked during drill except fuel/heat (continuous). See [docs/ARCHITECTURE.md](docs/ARCHITECTURE.md#drilling-system) for details.
